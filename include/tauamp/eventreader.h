...
template <TauDecayMode_t Mode_m = Rho, TauDecayMode_t Mode_p = Rho, ReadingLevel_t level = DETECTOR, bool DEBUG = false>
class EventReader_t {
public:
...
private:
    ...
    bool m_read_parton_level_event() {
        int n_charged_pion = m_branch_GenChargedPion->GetEntriesFast();
        int n_neutral_pion = m_branch_GenNeutralPion->GetEntriesFast();
        int n_neutrino = m_branch_GenNeutrino->GetEntriesFast();
        int n_beam_particle = m_branch_BeamParticle->GetEntriesFast();
        if (n_charged_pion != k_N_ChargedPion_TauM + k_N_ChargedPion_TauP) return false;
        if (n_neutral_pion != k_N_NeutralPion_TauM + k_N_NeutralPion_TauP) return false;
        if (n_neutrino != 2) return false;
        ...
        GenParticle *pic1 = (GenParticle *)m_branch_GenChargedPion->At(0);
        GenParticle *pic2 = (GenParticle *)m_branch_GenChargedPion->At(1);
        ...
        // * Neutrinos, using mother ID as well
        ...
        m_p_nu_tau_m_sol_1.SetPxPyPzE(num->Px, num->Py, num->Pz, num->E);
        m_p_nu_tau_p_sol_1.SetPxPyPzE(nup->Px, nup->Py, nup->Pz, nup->E);
        m_p_nu_tau_m_sol_2 = m_p_nu_tau_m_sol_1;
        m_p_nu_tau_p_sol_2 = m_p_nu_tau_p_sol_1;
        m_p_nu_tau_m_gen = m_p_nu_tau_m_sol_1;
        m_p_nu_tau_p_gen = m_p_nu_tau_p_sol_1;
    ...
    bool m_read_detector_level_event() {
        if constexpr (DEBUG) {
            bool good = m_read_parton_level_event();
            if (!good) return good;
        }
        int n_charged_pion = m_branch_ChargedPion->GetEntriesFast();
        ...
        GenParticle *beam_m = beamc1 < 0 ? beam1 : beam2;
        GenParticle *beam_p = beamc1 > 0 ? beam1 : beam2;
        double Ecm = beam_m->E + beam_p->E;

        GenParticle *pic1 = (GenParticle *)m_branch_ChargedPion->At(0);
        GenParticle *pic2 = (GenParticle *)m_branch_ChargedPion->At(1);
        ...
        TLorentzVector v_pip = pip->P4();
        TLorentzVector v_pim = pim->P4();
        ...
        // ** Calculating some parameters that will be used later
        double Etau = Ecm / 2.0;
        ...
        m_p_nu_tau_p_sol_2.SetPxPyPzE(p_nubar_sol_2.Pz(), p_nubar_sol_2.Py(), p_nubar_sol_2.Pz(), p_nubar_sol_2.E());
...
// template <ReadingLevel_t level>
// bool EventReader_t<Rho, Rho, level>::m_read_detector_level_event() {
//     int n_charged_pion = m_branch_ChargedPion->GetEntriesFast();
//     int n_neutral_pion = m_branch_NeutralPion->GetEntriesFast();
//     int n_beam_particle = m_branch_BeamParticle->GetEntriesFast();
//     if (n_charged_pion != k_N_ChargedPion_TauM + k_N_ChargedPion_TauP) return false;
...
//     TLorentzVector p_h_m = pi0m->P4();
//     TLorentzVector p_h_p = pi0p->P4();
//     TLorentzVector p_miss = pcm - (p_h_m + p_h_p);
//     double pm = p_miss.P();
//     double thetam = p_miss.Theta();
//     double phim = p_miss.Phi();
...
//     // * By first rotation around z-axis by -phim angle
//     // * then rotation around y-axis by -thetam angle
//     p_h_m.RotationZ(-phim);
//     p_h_m.RotationY(-thetam);
//     p_h_p.RotationZ(-phim);
//     p_h_p.RotationY(-thetam);
//     p_miss.RotationZ(-phim);
//     p_miss.RotationY(-thetam);
...
//     double ax = 4.0 * (x0 * x0 + y0 * y0);
//     double bx = 4.0 * x0 * (r0 * r0 + x0 * x0 + y0 * y0 - z02);
//     double cx = 2.0 * r0 * r0 * (x0 * x0 - y0 * y0 - z02) + pow(x0 * x0 + y0 * y0 - z02, 2);
//     double Delta = bx * bx - 4.0 * ax * cx;
//     if (Delta < 0) return false;
//     double x_sol_1 = (-bx + sqrt(Delta)) / 2.0 / ax;
//     double x_sol_2 = (-bx - sqrt(Delta)) / 2.0 / ax;
...
//     // * Rotate back to lab frame
//     p_nu_sol_1.RotationY(thetam);
//     p_nu_sol_1.RotationZ(phim);

//     p_nubar_sol_1.RotationY(thetam);
//     p_nubar_sol_1.RotationZ(phim);

//     p_nu_sol_2.RotationY(thetam);
//     p_nu_sol_2.RotationZ(phim);

//     p_nubar_sol_2.RotationY(thetam);
//     p_nubar_sol_2.RotationZ(phim);
...
// template <ReadingLevel_t level>
// bool EventReader_t<Rho, Rho, level>::m_read_parton_level_event() {
//     int n_charged_pion = m_branch_GenChargedPion->GetEntriesFast();
//     int n_neutral_pion = m_branch_GenNeutralPion->GetEntriesFast();
//     int n_neutrino = m_branch_GenNeutrino->GetEntriesFast();
//     int n_beam_particle = m_branch_BeamParticle->GetEntriesFast();
